...
### 步驟 4b：第三階段問卷（手動排序與最終篩選）
在向使用者詢問排序前，AI 代理必須將**步驟 4 中使用者勾選的指標**按順序整理出來，並為它們標上數字編號（例如 `1.`、`2.`、`3.` ...）。
...
```json
{
  "questions": [
    {
      "question": "請設定狀態列顯示順序 (目前已勾選：1. model-name, 2. quota...)。請在下方輸入框輸入以逗號分隔的數字序號或英文識別碼 (如：2, 1, memory-usage)。未填寫的指標將不予顯示。",
      "is_multi_select": false,
      "options": [
        "略過，使用原勾選順序啟用全部指標"
      ]
...
*   **解析與排序項目清單（混合模式解析）**：
    ...
    2.  **讀取步驟 4b 的排序輸入**：
        *   若使用者在步驟 4b 選擇了預設選項（"略過，使用原勾選順序啟用全部指標"、"Skip" 或其他語系的同義略過項），或者 Write-in 欄位為空白，則**直接使用步驟 4 得到的「已勾選英文識別碼陣列」**作為最終的順序。
